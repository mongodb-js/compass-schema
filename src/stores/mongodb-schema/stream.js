...
const TRUNCATE_STRINGS_AT = 1000; // chars
const STRING_EXAMPLE_VALUES_SET_SIZE = 100;
const EXAMPLE_VALUES_SET_SIZE = 500;
...
*/
const extractStringValueFromBSON = function(value) {
 ...
 */
const finalizeSchema = function(schema, parent, tag) {
  if (schema === undefined) {
    return schema;
  }
...
module.exports = function parse(options) {
  ...
  let semanticTypes = require('./semantic-types');

  if (_.isObject(options.semanticTypes)) {
    // enable existing types that evaluate to true
    const enabledTypes = _(options.semanticTypes)
      .pick(function(val) {
   ...
   */
  const getBSONType = function(value) {
    let T;
    if (value && value._bsontype) {
      T = value._bsontype;
    } else {
  ...
  const getSemanticType = function(value, path) {
   ...
   */
  const addToValue = function(type, value) {
    if (type.name === 'String') {
      if (value.length > TRUNCATE_STRINGS_AT) {
        value = value.slice(0, TRUNCATE_STRINGS_AT);
      }
    }
  ...
  const addToType = function(path, value, schema) {
    ...
    const typeName = (options.semanticTypes) ?
    ...
    // if the `storeValues` option is enabled, store some example values
      ...
      type.values = _.get(type, 'values', bsonType === 'String' ?
        new Reservoir(STRING_EXAMPLE_VALUES_SET_SIZE) : new Reservoir(EXAMPLE_VALUES_SET_SIZE));
   ...
   * @param {Object}  schema   the updated schema object
   */
  const addToField = function(path, value, schema) {
    const defaults = {};
  ...
  const parser = es.through(function write(obj) {
    // console.log('*** obj done', obj._id);
